...
func GetGenreRecommendation(watching *[]WATCHING, db *gorm.DB, preloadMovie []string, preloadTv []string, user *User) ([]Line_Render, []*GenreClassement) {
	...
	BestGenreLineRender := func(wg *sync.WaitGroup, channel chan Line_Render, bg *GenreClassement, db *gorm.DB, user *User, preloadMovie []string, preloadTv []string) {
		...
		if bg.ItemType == "movie" {
			req := db.Table("movies").Joins("INNER JOIN movie_genres ON movies.id = movie_genres.movie_id").Where("movie_genres.genre_id = ? AND movies.backdrop_image_storage_type != 2", bg.Genre.ID).Preload("WATCHING", "USER_ID = ? ", user.ID).Preload("PROVIDERS").Preload("WATCHLISTS", "id = ? ", user.ID).Limit(50).Order("RAND()")
			for _, p := range preloadMovie {
				req = req.Preload(p)
			}
			req.Find(&movies)
		} else {
			req := db.Table("tvs").Joins("INNER JOIN tv_genres ON tvs.id = tv_genres.tv_id").Where("tv_genres.genre_id = ? AND tvs.backdrop_image_storage_type != 2", bg.Genre.ID).Preload("WATCHING", "USER_ID = ? ", user.ID).Preload("PROVIDERS").Preload("WATCHLISTS", "id = ? ", user.ID).Limit(50).Order("RAND()")
